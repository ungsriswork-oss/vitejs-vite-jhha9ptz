    ...
    <div className="fixed inset-0 bg-black/50 z-[80] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 transform transition-all scale-100">
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium shadow-md transition-all"
          >
            ลบข้อมูล
          </button>
        </div>
...
const AdminLoginModal = ({ onClose, onLogin }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (password === 'DISCENTERYOM' || password === 'ManUng') {
      onLogin();
      onClose();
    } else {
      ...
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 transform transition-all scale-100">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Lock size={20} className="text-blue-600" /> เข้าสู่ระบบผู้ดูแล
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-md transition-all transform active:scale-95"
            >
              เข้าสู่ระบบ
            </button>
          </div>
...
const DrugCard = ({ drug, onClick }) => (
  <div
    ...
    className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-3 cursor-pointer hover:shadow-md transition-all flex items-center justify-between group"
  >
    <div className="flex items-center gap-4 overflow-hidden">
      ...
      >
        {drug.type === 'injection' ? <Syringe size={20} /> : <Pill size={20} />}
      </div>
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-slate-800 text-white p-4 flex justify-between items-center sticky top-0">
          <h2 className="text-xl font-bold">
            {initialData ? 'แก้ไขข้อมูลยา' : 'เพิ่มยาใหม่'}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="external">ยาใช้เฉพาะที่/ยาใช้ภายนอก</option>
              </select>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">
            ...
            <div className="col-span-2">
              ...
              >
                ...
                <option>
                  ยาสำหรับโครงการพิเศษของหน่วยงานของรัฐ (restricted list; R1)
                  [บัญชี จ.1 เดิม]
                </option>
                <option>
                  ยาพิเศษที่กำหนดแนวทางการใช้ยา (restricted list; R2) [บัญชี จ.2
                  เดิม]
                </option>
              </select>
            </div>
            ...
            {formData.type === 'injection' && (
              <>
                <div className="col-span-2 bg-rose-50 p-3 rounded-lg border border-rose-100 mt-2">
                  <p className="text-rose-700 text-sm font-bold mb-2">
                    ข้อมูลสำหรับยาฉีด
                  </p>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    สารละลายที่ใช้
                  ...
                  <textarea
                    name="diluent"
                    rows="3"
                    value={formData.diluent}
                    onChange={handleChange}
                    className="w-full p-2 border rounded-lg bg-white mb-2 resize-y"
                  />
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    ความคงตัว
                  </label>
                  <textarea
                    ...
                    onChange={handleChange}
                    className="w-full p-2 border rounded-lg bg-white resize-y mb-2"
                  />
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    วิธีการบริหาร
                  </label>
                  <textarea
                    ...
                    onChange={handleChange}
                    className="w-full p-2 border rounded-lg bg-white resize-y"
                    placeholder="เช่น รับประทานหลังอาหาร, IV drip 30 นาที..."
                  />
                </div>
              </>
            )}
            ...
            <FileUploader
              ...
              onFileSelect={(base64) =>
                setFormData((prev) => ({ ...prev, leaflet: base64 }))
              }
            />
          </div>
          <button
            onClick={() => onSave(formData)}
            className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center justify-center gap-2 mt-4"
          >
...
const DetailModal = ({ drug, onClose, onEdit, onDelete, isAdmin }) => {
  ...
  // --- ส่วนที่แก้ไข: ถ้าเปิด PDF อยู่ ให้แสดงเฉพาะ PDF (หน้าต่างเดิมจะไม่ถูกเรนเดอร์) ---
  if (showLeafletModal) {
    return (
      <div className="fixed inset-0 bg-slate-900 z-[9999] flex flex-col h-screen w-screen"> 
          ...
          <div className="flex justify-between items-center p-3 bg-slate-800 text-white shadow-md shrink-0">
             <div className="flex items-center gap-2 font-bold text-slate-200">
                <FileText size={20}/> <span>เอกสารกำกับยา: {drug.genericName}</span>
             </div>
             {/* ปุ่มปิด: พอกดแล้ว showLeafletModal เป็น false หน้าต่างเดิมจะกลับมาเอง */}
  ...
  // --- หน้าต่างปกติ (จะแสดงก็ต่อเมื่อ showLeafletModal เป็น false) ---
  return (
...
export default function App() {
  ...
  const handleSaveDrug = async (drugData) => {
    try {
      ...
      if (drugData.id) {
        ...
        const { id, ...dataToUpdate } = drugData;
        await updateDoc(docRef, dataToUpdate);
      } else {
        await addDoc(collRef, drugData);
      }
      setIsFormOpen(false);
      ...
      alert(
        'เกิดข้อผิดพลาดในการบันทึกข้อมูล (ขนาดรูปอาจใหญ่เกิน 1MB หรือไม่มีสิทธิ์เข้าถึง)'
      );
    }
  };
  ...
  const filteredDrugs = drugs.filter(
    (drug) =>
      (drug.genericName?.toLowerCase() || '').includes(
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans relative">
        ...
        <div className="max-w-md mx-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              title={isAdmin ? 'ออกจากโหมดผู้ดูแล' : 'เข้าสู่โหมดผู้ดูแล'}
            >
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="ค้นหาชื่อยา, ยี่ห้อ..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-slate-100 border-transparent focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-xl transition-all outline-none"
            />
          </div>
        </div>
      </header>
      <main className="max-w-md mx-auto p-4 pb-20">
        {permissionError && (
          <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 rounded shadow-sm flex items-start gap-3">
            <AlertTriangle size={24} className="shrink-0" />
            <div>
              <p className="font-bold">ไม่สามารถเข้าถึงข้อมูลได้</p>
              <p className="text-sm">
                กรุณาตั้งค่า <strong>Firestore Rules</strong> ใน Firebase
                Console ให้เป็น <code>allow read, write: if true;</code>
              </p>
              <a
                href={consoleUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-blue-600 hover:underline mt-2 text-sm font-semibold"
              >
                ไปที่หน้าตั้งค่า <ExternalLink size={14} />
              </a>
            </div>
          </div>
        )}
        {loading ? (
          <div className="text-center mt-10 text-slate-500 animate-pulse">
            กำลังโหลดข้อมูล...
          </div>
        ) : filteredDrugs.length > 0 ? (
          filteredDrugs.map((drug) => (
            <DrugCard key={drug.id} drug={drug} onClick={setSelectedDrug} />
          ))
        ) : (
          <div className="text-center text-slate-400 mt-10 flex flex-col items-center gap-3">
            <Pill size={48} className="opacity-20" />
            <p>ไม่พบข้อมูลยา</p>
            {drugs.length === 0 && isAdmin && (
              <button
      ...
      {isAdmin && (
        <div className="fixed bottom-6 right-6 z-40">
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all flex items-center gap-2"
          >
            <Plus size={24} />{' '}
            <span className="font-bold hidden md:inline">เพิ่มยา</span>
          </button>
        </div>
      )}
      ...
      {isFormOpen && isAdmin && (
        <DrugFormModal
          initialData={isEditing ? selectedDrug : null}
          onClose={() => {
            setIsFormOpen(false);
            setIsEditing(false);
          }}
      ...
      <ConfirmModal
        ...
        title="ยืนยันการลบ"
        message="คุณแน่ใจหรือไม่ที่จะลบข้อมูลยานี้? การกระทำนี้ไม่สามารถย้อนกลับได้"
      />
    </div>
  );
}
